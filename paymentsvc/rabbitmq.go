...
func (r RabbitMQ) Consume(ctx context.Context, consumer, qName, exName, routingKey string, kind Exchange, durable, autoDelete, internal, noWait bool, args amqp.Table, handler func([]byte) error) error {
	err := r.ExchangeDeclare(exName, kind, durable, autoDelete, internal, noWait, args)
	if err != nil {
		return err
	}

	q, err := r.QueueDeclare(qName, durable, autoDelete, false, noWait, args)
	if err != nil {
		return err
	}
